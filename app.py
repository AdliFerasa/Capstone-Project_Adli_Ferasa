...
def index():
    ...
    stats = {
        ...
        'rev_table' :  playstore.groupby(['Category', 'App']).agg({'Reviews':'sum','Rating':'mean'})\
            .sort_values('Reviews', ascending=False).head(10).reset_index().round(1).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
  ...
  #app yang paling banyak di download
    playstore.pivot_table(index = 'App',
           values = 'Installs').head(10).sort_values('Installs',ascending = False).plot(kind='barh')
    plt.ylabel('Aplikasi')
    plt.xlabel('Jumlah di-download')
